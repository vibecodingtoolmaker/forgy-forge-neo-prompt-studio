"""Run one bounded Forgy text generation with Forge-owned FLUX.2 Klein 4B."""
...
def main() -> int:
    ...
    try:
        ...
        namespace = runpy.run_path(
            ...
            run_name="forgy_flux2_klein_text_smoke",
        )
        context = namespace["MODEL_MANAGER"].resolve(sd_model)
        assert context.identity.adapter_id == "flux2-klein-qwen3-4b"
        ...
        if not visible:
            raise RuntimeError("FLUX.2 Klein Qwen3 produced no visible text")
        print("FLUX2_KLEIN_TEXT_GENERATION_OK")  # noqa: T201
        ...
        if generation_id and namespace is not None:
